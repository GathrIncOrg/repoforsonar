...
klera_meta_out = {
    "Project Area History": {
        "DSTID": "ProjectAreaHistoryV1.0.0",
        "Primary Key": {
            "isrowid": True,
            "isvisible": True,
            "datatype": "STRING"
...
# Start_Time= ""
count=1
for project_area_id, start_time in zip(Project_Area_Id, Start_Time):
    flag=1
    url = Instance_Url+f"/process/project-areas/{project_area_id}/history?startTime={start_time}&pageSize=100"
            ...
            for history_detail in history_entry:
                ...
                try:
                    ...
                    for activity in history_detail:
                        ...
                        kleradict.setdefault("History Entry Username", []).append(str(history_entry.attrib[list(history_entry.attrib.keys())[0].rsplit('}',1)[0]+'}user-name']))
                        kleradict.setdefault("Project Area Id", []).append(str(project_area_id))
                        kleradict.setdefault("Searched Before", []).append(str(start_time))
                        kleradict.setdefault("Primary Key", []).append(str(history_detail.attrib[list(history_detail.attrib.keys())[0].rsplit('}',1)[0]+'}url']+str(count)))
                        count=count+1
                        try:
                            kleradict.setdefault("History Detail Label", []).append(str(history_detail.attrib[list(history_detail.attrib.keys())[0].rsplit('}',1)[0]+'}label']))
                        except:
                            kleradict.setdefault("History Detail Label", []).append(None)

                        try:
                            kleradict.setdefault("History Detail Url", []).append(str(history_detail.attrib[list(history_detail.attrib.keys())[0].rsplit('}',1)[0]+'}url']))
                        except:
                            kleradict.setdefault("History Detail Url", []).append(None)

                        try:
                            kleradict.setdefault("History Detail Type", []).append(str(history_detail.attrib[list(history_detail.attrib.keys())[0].rsplit('}',1)[0]+'}type']))
                        except:
                            kleradict.setdefault("History Detail Type", []).append(None)

                        try:
                            kleradict.setdefault("History Detail Deferred", []).append(str(history_detail.attrib[list(history_detail.attrib.keys())[0].rsplit('}',1)[0]+'}deferred']))
                        except:
                            kleradict.setdefault("History Detail Deferred", []).append(None)

        
                        try:
                            kleradict.setdefault("Operation Type",[]).append(str(activity.tag.rsplit('}',1)[1]))
                        except:
                            kleradict.setdefault("Operation Type",[]).append(None)

                        try:
                            kleradict.setdefault("Title", []).append(str(activity.attrib[list(activity.attrib.keys())[0].rsplit('}',1)[0]+'}title']))
                        except:
                        ...
                        try:
                            kleradict.setdefault("Scope", []).append(str(activity.attrib[list(activity.attrib.keys())[0].rsplit('}',1)[0]+'}scope']))
                        except:
                        ...
                        if(len(kleradict['Project Area Id'])>100):
                            ...
                            out_dict = {'Project Area History': out_df}
                            klera_dst = [out_dict]
                            data_block = {}
